...
// Signal to the bootstrapper that the JS bundle has executed
if ((window as any).EBENEZER_BOOT_LOG) {
  (window as any).EBENEZER_BOOT_LOG.push("JS Module Execution Started");
}
    ...
    root.render(
      <React.StrictMode>
        <App onBooted={() => {
            ...
            // Remove bypass button if it exists
            document.getElementById('boot-bypass')?.remove();
        }} />
      </React.StrictMode>
    );
  } catch (error) {
    ...
    rootElement.innerHTML = `
      <div style="padding: 40px; font-family: 'Inter', sans-serif; text-align: center; max-width: 500px; margin: 0 auto; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh;">
        <div style="background: #fff; padding: 40px; border-radius: 32px; box-shadow: 0 20px 50px rgba(0,0,0,0.1); border: 1px solid #fee2e2;">
          ...
          <p style="color: #64748b; font-size: 13px; margin-bottom: 24px;">The application crashed during the render phase.</p>
          ...
          <button onclick="window.location.reload()" style="padding: 14px 28px; background: #2563eb; color: white; border: none; border-radius: 14px; font-weight: 800; cursor: pointer;">Restart</button>
        </div>
      </div>
    `;
  }
